...
const getCardSettings = () => {
  ...
  const cardOneDiv = getCheckbox('user-set-translate', 'перевод слова', true);
  const cardTwoDiv = getCheckbox('user-set-explanation', 'предложение с объяснением значения слова');
  const cardThreeDiv = getCheckbox('user-set-example', 'предложение с примером использования слова');
  const cardFourDiv = getCheckbox('user-set-transcription', 'транскрипция слова');
  const cardFiveDiv = getCheckbox('user-set-image', 'показывать изображение');
